...
Begin VB.Form frmSalesQuotation 
   ...
   Begin VB.Frame framDoc 
      ...
      Begin VB.ComboBox cmbQuoType 
         ...
         TabIndex        =   2
         ToolTipText     =   "Select the Sales Quotation Type"
         Top             =   315
         Width           =   750
      End
...
'
'  Sql = "select DocumentType from COM_DOCUMENTTYPE_MASTER where PertainingTo='Q'"
...
'     txtQuoNo = GetNo("SAL_QUOTATION_HDR", "QuotationType", "QuotationNo", Trim$(cmbQuoType), "Q", "SAL")
'  End If
'  ordtyperec.Close
...
Private Sub SetDocNo()
  ...
  txtQuoNo = GetDocNo("SAL_QUOTATION_HDR", "QuotationType", "QuotationNo", Trim$(cmbQuoType), "Q")
Catch ex As Exception
  Call ErrMsgShow(ex)
...
Private Sub CmdClear_Click()
  ...
  Form_Clear Me
  CurrDate Me
  Call LoadDocType(mSalQuotation)
  spItem.MaxRows = 1
...
tot1 = 0
  ...
  If optEnq Then
    ...
    sql = sql + " SAL_VW_ENQUIRY_DTL a where a.EnquiryType='" + Trim(txtEnqType) + "'"
    sql = sql + " and a.EnquiryNo = " & Trim(txtEnqNo) & ""
    ...
    If Not snap.EOF Then
      snap.MoveLast
snap.MoveFirst
      spItem.MaxRows = snap.RecordCount
...
Private Sub spItem_DblClick(sender as object, e as DblClickEventArgs)
  ...
  If Col = eItem.mItem And optdirect And tmp = "" Then
    ...
    If FetchValue_New(True, db.ConnectionString, sql, "11101010000", "List of Items for the Customer", "Item Code", "Item Description", "Part No", "UOM", "Rate") = True Then
      ...
      spItem.MaxRows = UBound(getvalue, 1) + 1
...
Private Sub InitializeSpreads()
	...
	Call Design_spItem("D:\OutOfSync\_2_UnicornERP_Convertion_VB6ToNextJS_Antigravity\Export_VB6_Sales\PrePost_Sales\Sales\Sales\SalesQuotation.frm", spItem, "")

	Call Design_sprCalc("D:\OutOfSync\_2_UnicornERP_Convertion_VB6ToNextJS_Antigravity\Export_VB6_Sales\PrePost_Sales\Sales\Sales\SalesQuotation.frm", sprCalc, "")
